...
class Simulator:
	def __init__(self, agent, model, num_trials, num_episodes, episode_length, sim_timestep,
			gui_active, gui_freq, agent_freq, reset_angle, reset_angle_variance):
		self.agent = agent
	...
	def run_experiment(self):
		for i in range(self.num_trials):	
			...
			for j in range(self.num_episodes):
				print "starting episode {} of {}".format(j+1, self.num_episodes)
...
if __name__ == "__main__":
	...
	parser.add_argument('--trials', type=int, default=10, help='number of trials to run (default: %(default)s)')
	parser.add_argument('--episodes', type=int, default=10, help="number of episodes to run (default: %(default)s)")
	parser.add_argument('--time', type=int, default=30, help="time length (seconds) of an episode (default: %(default)s)")
	parser.add_argument('--timestep', type=float, default=.001, help="delta timestep (seconds) for simulation (default: %(default)s)")
	parser.add_argument('--nogui', action="store_true", help="disables the gui")
	parser.add_argument('--gui_freq', type=int, default=25, help="gui update frequency (default: %(default)s)")
	parser.add_argument('--agent_freq', type=int, default=10, help="agent update frequency (default: %(default)s)")
	parser.add_argument('--track_length', type=int, default=2, help="length (meters) of the track (default: %(default)s)")
	parser.add_argument('--pole_length', type=float, default=0.3, help="length (meters) of the pole (default: %(default)s)")
	parser.add_argument('--reset_angle', type=float, default=0.0, help="average reset angle of the pendulum (default: %(default)s)")
	parser.add_argument('--reset_angle_variance', type=float, default=0.0, help="variance of reset angle of the pendulum (default: %(default)s)")
